        ...
        public string Post(string xml, string url, bool isUseCert, int timeout, HttpContext httpContext)
        {
            ...
            try
            {
                ...
                * ************************************************************/
                ...
                request.UserAgent = string.Format("WXPaySDK/{3} ({0}) .net/{1} {2}", Environment.OSVersion, Environment.Version, this._IWxPayConfig.GetMchID(), typeof(WxPayHttpService).Assembly.GetName().Version);
                request.Method = "POST";
                ...
                if (isUseCert)//是否使用证书--没用证书
                {
                    ...
                    X509Certificate2 cert = new X509Certificate2(path + this._IWxPayConfig.GetSSlCertPath(), this._IWxPayConfig.GetSSlCertPassword());
