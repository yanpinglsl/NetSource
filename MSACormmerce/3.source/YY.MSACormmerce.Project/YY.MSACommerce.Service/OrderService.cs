...
using YY.AgileFramework.Core;
using YY.AgileFramework.Common.IOCOptions;
using Newtonsoft.Json;
using YY.AgileFramework.Common.QueueModel;
using YY.AgileFramework.Common;
using DotNetCore.CAP;

namespace YY.MSACommerce.Service
{
        ...
        public long CreateOrder(OrderDto orderDto, UserInfo userInfo)
        {
            ...
            #region 异步清理购物车+延时取消任务+确认订单支付状态任务
            {
                ...
                try
                {
                    ...
                    //发布清理购物车任务
                    this._RabbitMQInvoker.Send(RabbitMQExchangeQueueName.OrderCreate_Exchange, message);
